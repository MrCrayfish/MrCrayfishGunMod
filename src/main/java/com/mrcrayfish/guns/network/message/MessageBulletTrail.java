...
public class MessageBulletTrail implements IMessage
{
    ...
    {
        this.positions = new Vec3d[spawnedProjectiles.length];
        this.motions = new Vec3d[spawnedProjectiles.length];
    ...
    public void encode(PacketBuffer buffer)
    {
        ...
        for(int i = 0; i < this.entityIds.length; i++)
        {
            ...
            Vec3d position = this.positions[i];
            buffer.writeDouble(position.x);
            buffer.writeDouble(position.y);
            buffer.writeDouble(position.z);

            Vec3d motion = this.motions[i];
    ...
    public void decode(PacketBuffer buffer)
    {
        ...
        this.positions = new Vec3d[size];
        this.motions = new Vec3d[size];
        for(int i = 0; i < size; i++)
        {
            this.entityIds[i] = buffer.readInt();
            this.positions[i] = new Vec3d(buffer.readDouble(), buffer.readDouble(), buffer.readDouble());
            this.motions[i] = new Vec3d(buffer.readDouble(), buffer.readDouble(), buffer.readDouble());
        }
