...
public class GunListOption<E extends IResourceLocation> extends AbstractOption
{
    ...
    {
        return new OptionButton(x, y, width, 20, this, this.getTitle().getFormattedText(), (button) -> {
            List<E> list = this.supplier.get();
            if(list.isEmpty())
                return;
            this.nextItem(Screen.hasShiftDown() ? -1 : 1);
            button.setMessage(this.getTitle().getFormattedText());
        }) {
            @Override
            public void renderButton(int mouseX, int mouseY, float partialTicks)
            {
                ...
                GunListOption.this.renderer.render(this, new MatrixStack(), partialTicks);
            }
        };
    }
