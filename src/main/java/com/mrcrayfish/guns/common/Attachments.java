...
{
    public static final Scope SHORT_SCOPE = Scope.builder().aimFovModifier(0.7F).reticleOffset(1.55F).viewFinderDistance(1.1).viewportFov(30.0).sightAnimation(SightAnimation.builder().setViewportCurve(Easings.LINEAR).setSightCurve(Easings.EASE_OUT_QUAD).setFovCurve(Easings.LINEAR).setAimTransformCurve(Easings.EASE_IN_QUAD)).modifiers(GunModifiers.SLOW_ADS).build();
