...
public class ProjectileExplosion extends Explosion
{
    ...
    public ProjectileExplosion(World world, Entity exploder, double x, double y, double z, float size, boolean causesFire, Mode mode)
    {
                ...
                for(int z = 0; z < 16; z++)
                {
                    if(x == 0 || x == 15 || y == 0 || y == 15 || z == 0 || z == 15)
                    {
                        ...
                        for(; f > 0.0F; f -= 0.225F)
                        {
                            ...
                            float resistance = Math.max(blockState.getExplosionResistance(this.world, pos, this.exploder, this), fluidState.getExplosionResistance(this.world, pos, this.exploder, this));
                            resistance = this.exploder.getExplosionResistance(this, this.world, pos, blockState, fluidState, resistance);
                            f -= (resistance + 0.3F) * 0.3F;

                            if(f > 0.0F && this.exploder.canExplosionDestroyBlock(this, this.world, pos, blockState, f))
                            {
                                set.add(pos);
                            }
