...
public abstract class EntityThrowable extends Entity implements IProjectile
{
    ...
    public void onUpdate()
    {
        ...
        {
            nextPosVec = new Vec3d(rayTraceResult.hitVec.x, rayTraceResult.hitVec.y, rayTraceResult.hitVec.z);
        }
        ...
        for(int i = 0; i < list.size(); ++i)
        {
            ...
            if(entity1.canBeCollidedWith())
            {
                ...
                {
                    flag = false;
                    AxisAlignedBB boundingBox = entity1.getEntityBoundingBox().grow(0.3D);
                    RayTraceResult result = boundingBox.calculateIntercept(posVec, nextPosVec);
                    if(result != null)
                    {
                        double d1 = posVec.squareDistanceTo(result.hitVec);
