                ...
                if(AimingHandler.get().isAiming() && !SyncedPlayerData.instance().get(mc.player, ModSyncedDataKeys.RELOADING))
                {
                    ...
                    if(modifiedGun.getModules().getZoom() != null)
                    {
                        ...
                        if(scope != null)
                        {
                            ...
                            {    newFov -= scope.getAdditionalZoom().getFovZoom() * (Config.COMMON.gameplay.scopeDoubleRender.get() ? 1:2); event.setNewfov(newFov + (1.0F - newFov) * (1.0F - (float) this.normalisedAdsProgress));}
                        }
